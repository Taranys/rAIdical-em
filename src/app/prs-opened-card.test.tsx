...
describe("PrsOpenedCard", () => {
  ...
  it("shows empty state when no data", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/No opened PRs found/)).toBeInTheDocument();
    });
  });
  ...
  it("renders bar chart when data is available", async () => {
    vi.spyOn(global, "fetch").mockResolvedValue({
      json: async () => ({
        byMember: [
          { author: "alice", count: 7 },
          { author: "bob", count: 4 },
        ],
        byWeek: [
          { week: "2026-W06", count: 5 },
          { week: "2026-W07", count: 6 },
        ],
      }),
    } as Response);
  ...
  it("shows total PR count in description", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/11 PRs opened/)).toBeInTheDocument();
    });
  });
});
