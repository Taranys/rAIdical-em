...
- [ ] Each team member is shown as a card with: avatar, display name, overall maturity level badge
- [ ] Expanding a member card reveals two sections:
...
- [ ] A radar/spider chart visualizes the balance across all dimensions for each team member (e.g., strong on architecture and pedagogy, room to grow on security and boldness)
- [ ] A toggle allows switching between "Technical only", "Soft skills only", and "All dimensions" views
